...
/**
 * @author super_lc
 * @version 创建时间：2018年12月1日 下午8:14:44
 * @filename SupInfoFrame.java
 * @ClassName 类名称
 * @Description 类描述
 */
public class SupInfoFrame extends JFrame {
		...
		try {
			ps = con.prepareStatement(
					"select a.sup_number,sup_name,sup_address,sup_tel,part_number,sup_provide ,sup_introduce "
							+ "from supplier as a,sup_provide as b where a.sup_number=b.sup_number ");
			rs = ps.executeQuery();
			rs.last();// 将光标置于最后一行
...
//						ps = con.prepareStatement("insert into supplier value (" + a1 + ", '" + a2 + "', '" + a3 + "', "
//								+ a4 + ",'" + a7 + "');");
