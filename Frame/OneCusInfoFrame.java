...
public class OneCusInfoFrame extends JFrame {
	...
	private void init() {
		...
		try {
			...
			ps = con.prepareStatement(
					"select a.cus_number,cus_name,cus_address,cus_tel,part_number,cus_need from customer as a,cus_demand as b where a.cus_number=b.cus_number and a.cus_number = "
							+ LoginFrame.id);
			rs = ps.executeQuery();
			rs.last();// 将光标置于最后一行
...
//						int a4 = Integer.parseInt((String) table.getModel().getValueAt(n, 3));
						...
						// ps = con.prepareStatement("insert into cus_demand values(" + a1 + "," + a5 +
						// "," + a6 + ");");
//						ps = con.prepareStatement(
